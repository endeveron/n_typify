...
import { CognitiveFnCard, TraitCard } from '@/core/types/mbti';
...
const MBTIDashboard = ({ langCode }: TMBTIDashboardProps) => {
  ...
  // console.log('translation', translation?.energyCard);
  ...
  return (
    <div className="mbti-dashboard flex flex-1 flex-col justify-end">
      {/* Trait Cards */}
      {traitCards.length ? (
      ...
      {cognitiveFnCards.length ? (
        <div className="my-6 flex flex-wrap items-center justify-center gap-1">
          {cognitiveFnCards.map((data) => (
            <MBTICognitiveFunctionCard {...data} key={data.title} />
          ))}
        </div>
      ) : null}
    </div>
